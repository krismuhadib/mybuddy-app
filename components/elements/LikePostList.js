...
const LikePostList = ({ updatePostLike, page, getAllPost, item }) => {
  ...
  if (count > 0 && !item.favorites.includes(animalData._id)) {
    heartColor = 'red';
  ...
  return (
    <View key={i}>
      {animalData && (
        <View style={{ flexDirection: 'column', alignItems: 'center', alignContent: "center", justifyContent:"flex-start"}}>
          <TouchableOpacity
