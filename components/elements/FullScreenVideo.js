...
import PinchableBoxFullScreen from './PinchableBoxFullScreen';


const ScreenWidth = Math.round(Dimensions.get('window').width);
const ScreenHeight = Math.round(Dimensions.get('window').height);

const FullScreenVideo = ({ uri, item, visible, onClose }) => {

  const [modalVisible, setModalVisible] = useState(false);
  const videoUri = config.linkserver + item.animal_id._id + '/images/posts/videos/' + item.video_id + '.mp4';
  const imageUri = config.linkserver + item.animal_id._id + '/images/posts/' + item.image_id + '.jpg';
  const [hasMediaLibraryPermission, setHasMediaLibraryPermission] = useState(null);
  const [modalPosition, setModalPosition] = useState(null);

  console.log("FULLSCREEN item 22")
  ...
  return (
    ...
    >
      <View style={styles.container}>
        ...
        {/* Bouton de fermeture */}
        <View style={{ position: "absolute", marginTop: Platform.OS === 'android' ? -10 : 0, zIndex: 900, flex: 1, width: "100%", flexDirection: "row", justifyContent: "space-between", alignContent: "space-between" }}>
        ...
        {(modalVisible) &&
          ...
          <View style={{ position: "absolute", zIndex: 1000, top: ScreenHeight - ScreenHeight / 1.6, flex: 1, backgroundColor: "transparent", alignContent: "center", justifyContent: "center", alignItems: "center" }}>
        ...
        onPress={onClose}
              activeOpacity={1}
              >
          <View style={{ marginTop: ScreenHeight / 4, alignContent: "center", justifyContent: "center", position: "relative" }}>
            <PinchableBoxFullScreen imageUri={config.linkserver + item.animal_id._id + '/images/posts/' + item.image_id + '.jpg'}
              item={item}
