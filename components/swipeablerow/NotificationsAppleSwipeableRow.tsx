...
export default class ProfileAppleSwipeableRow extends Component {
  ...
  renderRightActionDelete = (text, color, x, progress) => {
   //console.log("renderRightActionDelete23",this.props )
    ...
    const pressHandler = () => {
      ...
      this.DeleteNotification(this.props.children);
      this.close();
      // Refresh flatList
  ...
  DeleteNotification = (item) => {
    console.log("remove notif button item", item)
    ...
    fetch(config.uri+'notifications/deletenotifications', {
    //fetch('http://localhost:3000/users/favorites', {
      ...
      body: JSON.stringify({
        ...
        user_id : this.props.children.props.item.user_id,
        notification_id : this.props.children.props.item._id,
    ...
    .then((res) => { 
      if (res.success === true) {
        ...
        alert('Problem with Delete Notifications');
      }
    });
  }; 
  ...
  DeleteAnimal = () => {
    ...
    fetch(config.uri+'animals/deleteanimal', {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
