...
export default class FriendsAppleSwipeableRow extends Component {
  ...
  renderRightActionDelete = (text, color, x, progress) => {
   //console.log("renderRightActionDelete23",this.props )
    ...
    const pressHandler = () => {
      console.log("this.props.onRefresh.onRefresCCC();",this.props.children  )
        
      this.ButtonRemoveFriends(this.props.children);
      this.close();
      // Refresh flatList
  ...
  ButtonRemoveFriends = async (item) => {

    console.log("remove friend button itemAAABBB", item)
    ...
    //fetch('http://localhost:3000/users/favorites', {
      ...
      body: JSON.stringify({
        ...
        friend_id : item.props.friend_id._id,
