...
const AlertDetailsScreen = (route) => {
  ...
  const [allowNewAlert, setAllowNewAlert] = useState(true);
  ...
  useEffect(() => {
    ...
    const sendPusAlertNotification = async () => {
     ...
     // const commentary_elliptic = commentary.length > 40 ? commentary.substring(0, 40) + "..." : commentary;
      ...
      try {
        const response = await fetch(config.uri + 'notifications/sendalertnotifications', {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            ...
            originname: animalData.name,
            postanimalid: animalData._id,
            latitude: latitude,
            longitude: longitude,
  ...
  const inputValidate = (text, type) => {
    ...
    if (type == 'title') {
      ...
      if (text.length > config.maxLength.characters) {
        //setAlertDescription(false);
      ...
      <Modal
        ...
        <View style={BDiaryStyles.centeredView}>
          <View style={BDiaryStyles.modalView}>
            ...
            {(allowNewAlert === false) && 
            <View>
              <Text style={BDiaryStyles.modalText}> {i18n.t('alertDetails.maxAlert')}</Text>
            </View>
            }
