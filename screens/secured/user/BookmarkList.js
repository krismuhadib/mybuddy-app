...
const BookmarkList = (route) => {
  ...
  const getAllPost = async () => {
    ...
    if (animalData && animalData._id) {
      fetch(config.uri + 'posts/getbookmarkposts', {
        method: 'POST',
        headers: {
...
<HeaderBuddyLeft 
            ...
            label={i18n.t('bookmarksList.title')}
            navigationName="User"
            navigationFrom= "User"
            goBack={true}
            />
      ...
      {(postList.length === 0) &&
        <EmptyListMessage
          text={i18n.t('Error.No_Bookmarks')}
        />
      }
