...
import { SafeAreaView, Platform, KeyboardAvoidingView, ScrollView,
  TextInput, StyleSheet, Dimensions, View, Text, TouchableOpacity } from 'react-native';
...
const NewAnimalProfileScreen = () => {
  ...
  const [name, setName] = useState(animalData.name);
  ...
  const [description, setDescription] = useState(animalData.decription);
  ...
  }, [sterilisation, name, genre, keyboardOffset, scrollEnabled, bufferScrollViewHeight]);
            ...
            <View style={BDiaryStyles.formsContainer}>
              ...
              <View style={BDiaryStyles.inputContainer}>
                <TouchableOpacity style={BDiaryStyles.inputContainerRow}
                  ...
                  <Text style={[BDiaryStyles.h5, { flex: 1, textTransform: "capitalize", color: Colors.greyM }]}>{animalData.breedname}</Text>
                  <Feather style={{ paddingRight: 10 }} name="chevron-down" size={25} color={Colors.greyL} />
                </TouchableOpacity>
              </View>
...
export default NewAnimalProfileScreen;
