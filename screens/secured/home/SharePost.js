...
import { SectionList, Image, FlatList, Dimensions, StyleSheet, View, Text, TouchableOpacity } from 'react-native';
...
const SharePostScreen = ({ route }) => {
  ...
  console.log("SharePost from :", params.from)

  useEffect(() => {
    getAllFriends();
    getAllAnimals();
  }, [isFetching, typeOf, params,]);
  ...
  const buttonSendMessage = async (item) => {
    await fetch(config.uri + 'messages/create_userchatroom', {
      ...
      .then((res) => {
        if (res.success === true) {
          //console.log("ChatRoom", res.favoris);
          setRoomId(res.room_id);
         
          if (roomId) {
            console.log('Create USER CHATRROMM room_id', res.room_id);

            //console.log("send meeqge Button item", item);
            // if (from === "SendScreen") {

            //   navigation.navigate('MessageBottomScreen', {
            ...
            // }
              ...
              navigation.navigate('SendPostMessage', {
                from: "Home",
